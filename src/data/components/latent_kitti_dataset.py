...
class LatentVectorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (torch.from_numpy(latent_vector).to(torch.float32), torch.from_numpy(rot).to(torch.float32), torch.from_numpy(w).to(torch.float32)), torch.from_numpy(gt).to(torch.float32).squeeze()
